...
# 調べたいitemIDを代入することステータスを収集できる.今回は11111とした.
QUERY= sudo mysql zabbix -e "SELECT from_unixtime(clock),value FROM history_uint where itemid = '11111' order by clock desc limit 100; "
# シェルを実行、実行ログを受け取る
VALUE=` echo ${QUERY} `
