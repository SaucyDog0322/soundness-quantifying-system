...
# 調べたいitemIDを代入することステータスを収集できる.今回は11113とした.
QUERY= sudo mysql zabbix -e "SELECT from_unixtime(clock),value FROM history where itemid = '11113' order by clock desc limit 100; "
# シェルを実行、実行ログを受け取る
VALUE=` echo ${QUERY} `
